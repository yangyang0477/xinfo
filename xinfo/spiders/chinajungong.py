...
class ChinajungongSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        item['title'] = response.xpath(
            "//div[@class = 'mainnew_l']//h1/text()").extract()
        item['news_date'] = response.xpath(
            "//span[@class = 'news_info_time']/text()").extract()  # 发布时间
        item['content'] = response.xpath(
            "//div[@class = 'news_info_content']//text()").extract()  # 正文
        item['news_url'] = response.url  # url
        item['crawl_date'] = time.strftime('%Y-%m-%d')
        item['referer_web'] = response.xpath(
            "//span[@class='news_info_from']/text()").extract()  # 引用的网站名
